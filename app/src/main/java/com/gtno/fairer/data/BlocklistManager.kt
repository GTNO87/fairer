...
internal object BlocklistManager {
    ...
    // Both fields are published together in a single volatile write so no
    // VPN thread can ever see the new blocklist paired with the old category map.
    private data class State(
        val blocked: HashSet<String>,
        val categories: HashMap<String, String>,
    )
    @Volatile private var state = State(hashSetOf(), hashMapOf())
    ...
    fun isBlocked(domain: String): Boolean {
        ...
        var d = domain
        while (d.isNotEmpty()) {
            if (s.blocked.contains(d)) return true
            val dot = d.indexOf('.')
            if (dot < 0) break
            d = d.substring(dot + 1)
        }
    ...
    fun blockResultFor(domain: String): String? {
        ...
        var d = domain
        while (d.isNotEmpty()) {
            if (s.blocked.contains(d)) return s.categories[d] ?: "Other"
            val dot = d.indexOf('.')
            if (dot < 0) break
            d = d.substring(dot + 1)
        }
    ...
    fun domainCount(): Int = state.blocked.size
